...
const RegionSelectSection = () => {
  ...
  const handleClick = (cityValue: string) => {
    const districts = districtMap[cityValue] || [];

    // "전체" 항목 제외하고 모든 구를 도시-구 형식으로 변환
    const regionParams = districts
      .filter((d) => d.value !== "ALL")
      .map((d) => `region=${cityValue}-${d.value}`)
      .join("&");

    const basePath = selectedTab === "공인중개사" ? "/agent" : "/service";

    router.push(`${basePath}?${regionParams}`);
  };

  return (
    <section className="w-full bg-background-extraSoft py-[60px] md:py-[100px] flex justify-center">
      <div className="w-full px-[15px] lx:px-20 md:max-w-screen-xl2">
        {/* 타이틀 + 탭 */}
        <div className="flex flex-col gap-3 md:gap-0 md:flex-row justify-between md:items-start items-center mb-10 md:mb-14">
